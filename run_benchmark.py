...
def run_prediction(model, benchmark_folder, out_folder, baseline=True, old_ssn=False):
    def run_one_folder(model, folder, out_folder):
        ...
        for mf in tqdm(model_folders):
            ...
            for gt in gts:
                ...
                img = ssn_touch_pred(model, mask, ibl, device, baseline, old_ssn)
...
if __name__ == '__main__':
    ...
    model.to(device)
    weight_file = 'weights/best_weights/baseline_human_20-May-11-37-AM.pt'
    checkpoint = torch.load(weight_file, map_location=device)
    model.load_state_dict(checkpoint['model_state_dict'])

    benchmark_folder = '/home/ysheng/Dataset/benchmark_ds'
    exp_name = 'old_baseline_human'
    out_folder = join(benchmark_folder, exp_name)
    os.makedirs(out_folder, exist_ok=True)

    run_prediction(model, join(benchmark_folder,'shadow_gt'), out_folder,old_ssn=True)